...
class SwiftHeapTest: XCTestCase {
    ...
    fileprivate func verifyMaxHeap(_ h: SwiftHeap<Int>) -> Bool {
        for i in 0..<h.count {
            let left = h.leftChildIndex(ofIndex: i)
            let right = h.rightChildIndex(ofIndex: i)
            let parent = h.parentIndex(ofIndex: i)
            if left < h.count && h.inner[left] > h.inner[i] { return false }
            if right < h.count && h.inner[right] > h.inner[i] { return false }
            if i>0 && h.inner[parent] < h.inner[i] { return false }
        }
        return true
    }
    
    fileprivate func verifyMinHeap(_ h: SwiftHeap<Int>) -> Bool {
        for i in 0..<h.count {
            let left = h.leftChildIndex(ofIndex: i)
            let right = h.rightChildIndex(ofIndex: i)
            let parent = h.parentIndex(ofIndex: i)
            if left < h.count && h.inner[i] > h.inner[left] { return false }
            if right < h.count && h.inner[i] > h.inner[right] { return false }
            if i > 0 && h.inner[parent] > h.inner[i] { return false }
        }
        return true
    }
    ...
    func testMaxHeapOneElement() {
        let heap = SwiftHeap<Int>(array: [10], sort: >)
    ...
    func testCreateMinHeap() {
        ...
        XCTAssertEqual(h1.inner, [1, 2, 3, 4, 5, 6, 7])
        XCTAssertFalse(h1.isEmpty)
        XCTAssertEqual(h1.count, 7)
        ...
        XCTAssertEqual(h2.inner, [1, 3, 2, 4, 6, 7, 5])
        XCTAssertFalse(h2.isEmpty)
        XCTAssertEqual(h2.count, 7)
        ...
        XCTAssertEqual(h3.inner, [1, 2, 3, 4, 7, 9, 10, 14, 8, 16])
        XCTAssertFalse(h3.isEmpty)
        XCTAssertEqual(h3.count, 10)
        ...
        XCTAssertEqual(h4.inner, [0, 4, 1, 5, 12, 8, 3, 16, 7, 17, 13, 10, 9, 27])
        XCTAssertFalse(h4.isEmpty)
        XCTAssertEqual(h4.count, 14)
    ...
    func testCreateMaxHeapEqualElements() {
        let heap = SwiftHeap(array: [1, 1, 1, 1, 1], sort: >)
        XCTAssertTrue(verifyMaxHeap(heap))
        XCTAssertTrue(verifyMinHeap(heap))
        XCTAssertEqual(heap.inner, [1, 1, 1, 1, 1])
    }
    ...
    func testRemoving() {
        var h = SwiftHeap(array: [100, 50, 70, 10, 20, 60, 65], sort: >)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [100, 50, 70, 10, 20, 60, 65])
        ...
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [100, 50, 70, 10, 20, 65])
        
        let v2 = h.removeAt(4)
        XCTAssertEqual(v2, 20)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [100, 65, 70, 10, 50])
        
        let v3 = h.removeAt(4)
        XCTAssertEqual(v3, 50)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [100, 65, 70, 10])
        
        let v4 = h.removeAt(0)
        XCTAssertEqual(v4, 100)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [70, 65, 10])
        
        XCTAssertEqual(h.peek()!, 70)
        let v5 = h.remove()
        XCTAssertEqual(v5, 70)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [65, 10])
        
        XCTAssertEqual(h.peek()!, 65)
        let v6 = h.remove()
        XCTAssertEqual(v6, 65)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [10])
        
        XCTAssertEqual(h.peek()!, 10)
        let v7 = h.remove()
        XCTAssertEqual(v7, 10)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [])
    ...
    func testRemoveRoot() {
        var h = SwiftHeap(array: [15, 13, 9, 5, 12, 8, 7, 4, 0, 6, 2, 1], sort: >)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [15, 13, 9, 5, 12, 8, 7, 4, 0, 6, 2, 1])
        XCTAssertEqual(h.peek()!, 15)
        let v = h.remove()
        XCTAssertEqual(v, 15)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [13, 12, 9, 5, 6, 8, 7, 4, 0, 1, 2])
    }
    
    func testRemoveRandomItems() {
        for n in 1...40 {
            var a = randomArray(n)
            var h = SwiftHeap(array: a, sort: >)
            XCTAssertTrue(verifyMaxHeap(h))
            XCTAssertTrue(isPermutation(a, h.inner))
    ...
    func testInsert() {
        ...
        h.insert(10)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [15, 13, 10, 5, 12, 9, 7, 4, 0, 6, 2, 1, 8])
    }
    
    func testInsertArrayAndRemove() {
        var heap = SwiftHeap<Int>(sort: >)
        heap.insert([1, 3, 2, 7, 5, 9])
        XCTAssertEqual(heap.inner, [9, 5, 7, 1, 3, 2])
    ...
    func testReplace() {
        var h = SwiftHeap(array: [16, 14, 10, 8, 7, 9, 3, 2, 4, 1], sort: >)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [16, 14, 10, 8, 7, 9, 3, 2, 4, 1])
        
        h.replace(index: 5, value: 13)
        XCTAssertTrue(verifyMaxHeap(h))
        XCTAssertEqual(h.inner, [16, 14, 13, 8, 7, 10, 3, 2, 4, 1])
